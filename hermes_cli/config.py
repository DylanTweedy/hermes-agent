...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        "discover_models", "extra_body", "request_overrides",
    }
    ...
    def _normalize_request_overrides(raw_overrides: Any) -> Dict[str, Any] | None:
        if not isinstance(raw_overrides, dict):
            return None

        normalized_overrides: Dict[str, Any] = {}
        unexpected = sorted(set(raw_overrides) - {"max_tokens", "reasoning_effort"})
        if unexpected:
            logger.warning(
                "providers.%s: unsupported request_overrides keys ignored: %s",
                provider_key or "?", ", ".join(unexpected),
            )

        max_tokens = raw_overrides.get("max_tokens")
        if max_tokens is not None:
            if isinstance(max_tokens, int) and max_tokens > 0:
                normalized_overrides["max_tokens"] = max_tokens
            else:
                logger.warning(
                    "providers.%s: invalid request_overrides.max_tokens ignored: %r",
                    provider_key or "?", max_tokens,
                )

        reasoning_effort = raw_overrides.get("reasoning_effort")
        if reasoning_effort is not None:
            parsed = parse_reasoning_effort(str(reasoning_effort))
            if parsed is None:
                logger.warning(
                    "providers.%s: invalid request_overrides.reasoning_effort ignored: %r",
                    provider_key or "?", reasoning_effort,
                )
            else:
                normalized_overrides["reasoning_effort"] = str(reasoning_effort).strip().lower()
    ...
    request_overrides = _normalize_request_overrides(entry.get("request_overrides"))
    if request_overrides:
        normalized["request_overrides"] = request_overrides
